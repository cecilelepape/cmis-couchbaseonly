...
public class StorageFactory {
	...
	static public StorageService createStorageService(Map<String,String> parameters) throws CmisConstraintException{
		...
		if(COUCHBASE.equals(storageType)) return createCouchbaseStorageService(parameters);
	...
	static private StorageService createCouchbaseStorageService(Map<String,String> parameters) throws CmisConstraintException{
		String storageType = parameters.get(STORAGE);
		if(! COUCHBASE.equals(storageType)) return null;
		String bucket = parameters.get(BUCKET);
		if(bucket==null) throw new CmisConstraintException("Couchbase storage bucket is undefined");
		return new CouchbaseStorageService(bucket);
	}
