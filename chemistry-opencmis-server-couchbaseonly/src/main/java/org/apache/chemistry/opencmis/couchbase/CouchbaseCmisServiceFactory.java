		...
		for (String key : keys) {
			...
			if (key.startsWith(PREFIX_LOGIN)) {
				...
				if (key.endsWith(SUFFIX_READWRITE)) {
					...
					CouchbaseService cbService = new CouchbaseService(metadataBucket);
					...
					CouchbaseRepository cbr = repositoryManager
							.getRepository(repositoryId);
					String storageBucket = parameters.get(key);
					System.out.println("Config storage bucket " + storageBucket);

					
					Map<String, String> storageMap = new HashMap<String, String>();
					storageMap.put(StorageFactory.STORAGE, "couchbase");
					storageMap.put(StorageFactory.BUCKET, storageBucket);
