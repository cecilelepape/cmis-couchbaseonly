...
public class TestCouchbaseRepository {
	...
	static final String bucketStorage = "cmiscouchbase";
	...
	static public void before(){
		cbService = new CouchbaseService(bucketId);
		storageService = new CouchbaseStorageService(bucketStorage);
	}
	...
	public void testCreateFolderAndDocuments() {
		...
		try {
			...
			System.out.println("Adding a subfolder folderA");
			...
			System.out.println("Adding a document in folderA");
			...
			ContentStream stream = storageService.getContent(docDataB.getId(), null, null, null);
			assertNotNull("doc1.txt content cannot be retrieved", stream);
			assertTrue("content length is not valid",stream.getLength() == cStream.getLength()); 
